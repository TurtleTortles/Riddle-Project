...
def intro2(ch_type, ins, event, tip):
    print('如果你要幫忙，打instructions, 如果你想要看到隨機事件，打events, 如果你想要tips，如果你要開始，打play')
    while True:
        select = input()
...
def main():
    ...
    instruction = ['''這個謎語的遊戲就是一個跟你和另外一個人比誰可以先猜謎語的答案。第一個人會先猜。如果他猜對了，他就得到一分。
    如果他沒大隊，另外一個人就可以猜。遊戲開始的時候，你就可以選如果你要玩這個遊戲用繁體或簡體。如果你先要用繁體玩，打——。
    如果你想要用簡體玩，打——。你選完以後，遊戲就開始。遊戲開始的時候，你就可以看到密語和可以開始猜一猜。小心！
    ...
    if words == 'trd':
        ...
        Answers = sim_Answers
        ch_set = 1
    intro2(ch_set, instruction, event_ins, tipntrick)
